...
class JElementArticle extends JElement
{
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$article = JTable::getInstance('swaplocal');
		if ($value) {
			$article->load($value);
		} else {
			$article->title = JText::_('COM_SWAPLOCAL_SELECT_AN_ARTICLE');
		}
		...
		$link = 'index.php?option=com_swaplocal&amp;task=element&amp;tmpl=component&amp;function=jSelectArticle_'.$name;
...
//		$html .= "\n &#160; <input class=\"inputbox modal-button\" type=\"button\" value=\"".JText::_('JSELECT')."\" />";
		$html .= '<div class="button2-left"><div class="blank"><a class="modal" title="'.JText::_('COM_SWAPLOCAL_SELECT_AN_ARTICLE').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}">'.JText::_('JSELECT').'</a></div></div>'."\n";
