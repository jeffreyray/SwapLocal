 ...
 * @since		1.6
 */
class SwapLocalModelArticle extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		if (!empty($record->id)) {
			...
			return $user->authorise('core.delete', 'com_swaplocal.article.'.(int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_swaplocal.article.'.(int) $record->id);
		}
	...
	*/
	public function getTable($type = 'SwapLocal', $prefix = 'JTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_swaplocal.article', 'article', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_swaplocal.edit.article.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('article.id') == 0) {
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_swaplocal.articles.filter.category_id')));
			}
		}
